...
public:
   ...
   */
  void setRecvTimeout(int timeout_ms) noexcept(false);

  /**
   * @brief 带超时的接收方法
   * @param buffer 接收缓冲区
   * @param bufferLen 缓冲区长度
   * @param sourceAddress 返回源地址
   * @param sourcePort 返回源端口
   * @param timeout_ms 超时时间(毫秒)
   * @return 接收到的字节数，超时返回0，错误返回-1
   */
  int recvFromWithTimeout(void *buffer, int bufferLen,
                          std::string &sourceAddress,
                          unsigned short &sourcePort,
                          int timeout_ms) noexcept(false);

  /**
   * @brief 检查socket是否有数据可读
   * @param timeout_ms 等待超时时间
   * @return true 表示有数据可读
   */
  bool poll(int timeout_ms) noexcept(false);
