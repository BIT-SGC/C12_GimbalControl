...
CommunicatingSocket::CommunicatingSocket(int type, int protocol) noexcept(false)
...
void UDPSocket::setRecvTimeout(int timeout_ms) noexcept(false) {
#ifdef WIN32
  DWORD tv = timeout_ms;
#else
  ...
  if (setsockopt(sockDesc, SOL_SOCKET, SO_RCVTIMEO, (raw_type *)&tv,
                 sizeof(tv)) < 0) {
    throw SocketException("Set socket timeout failed", true);
  }
}

bool UDPSocket::poll(int timeout_ms) noexcept(false) {
  fd_set read_fds;
  FD_ZERO(&read_fds);
  FD_SET(sockDesc, &read_fds);

  struct timeval tv{.tv_sec = timeout_ms / 1000,
                    .tv_usec = (timeout_ms % 1000) * 1000};

  int ret = select(sockDesc + 1, &read_fds, nullptr, nullptr, &tv);
  if (ret < 0)
    throw SocketException("select failed", true);
  return ret > 0;
}

int UDPSocket::recvFromWithTimeout(void *buffer, int bufferLen,
                                   std::string &sourceAddress,
                                   unsigned short &sourcePort,
                                   int timeout_ms) noexcept(false) {
  if (!poll(timeout_ms))
    return 0; // 超时
